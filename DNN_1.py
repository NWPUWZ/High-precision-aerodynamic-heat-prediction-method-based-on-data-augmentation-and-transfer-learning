# -*- coding: utf-8 -*-
"""
Created on Fri Apr 16 19:07:42 2022
...
# %%
    ...
    class DeepReZero(nn.Module):
        def __init__(self, input_num, width, depth, output_num):
            """
            input_num: int 输入的特征数目
            width: int 隐藏层宽度
            depth: int 隐藏层深度
            output_num: int 输出层数目
            此类是残差神经网络，在使用全连接神经网络时不用管
            """
    ...
    class Trainer():
        ...
        def __init__(self, model, batch,lr, L2, trainset,valset):
            ...
            初始化，读取数据，构建模型
            """
            # 分割数据为训练集与验证集/测试集, 测试集占数据的20%
            # 此部分需要修改
            #trainset, valset = train_test_split(data, test_size=0.05)
            # 将numpy数组变为张量，pytorch中要使用张量数据
        ...
        def val(self):
            ...
            return np.array(val_loss).mean()#为什么val_loss要求均值,对一系列batch求的均值
    
        def save(self, **params):
            """
            保存模型训练参数
            params : 字典文件，包含模型的必要参数
            """
    ...
    if __name__ == '__main__':
        # %%
        # 准备读取训练数据，假设数据保存为train.dat，数据需要提前准备
        # 这里的train.dat是(100, 11)的随机数据
       ...
       # train_Data[:,10]=np.log10(train_Data[:,10])
        # 训练数据一般需要归一化(min-max)或者标准化(mean-std)
        # 假设是归一化，归一化参数读取如下：
        ...
        # %%
        batch = 64  # batch size，建议为2的指数
        lr = 0.00001    # 学习率
        L2 = 0.00001    # L2正则化系数
        epoch = 1600   # 训练次数
        bn = False    # 是否添加bn层
        drop = False    # 是否添加dropout
        layer = (5,64,64,64,64,1)    # 网络结构
        inputrange = layer[0]    # 输入特征的个数
